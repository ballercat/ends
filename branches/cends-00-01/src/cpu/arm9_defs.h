#ifndef ENDSARM9DEFSH
#define ENDSARM9DEFSH

/* All of these are just to help avoid about a thousand
   or so bit-masks, and about a 100 macros */
#include "../types.h"

#define RD (cpu->R[cpu->op.Rd])
#define RS (cpu->R[cpu->op.Rs])
#define RN (cpu->R[cpu->op.Rn])
#define RM (cpu->R[cpu->op.Rm])
#define LR (cpu->R[14])
#define PC (cpu->R[15])
...
#define ASR(n,b) ( (signed)(n) >> (b) )
#define LSL(n,b) ( (n) << (b) )
#define LSR(n,b) ( (n) >> (b) )
#define ROR(n,b) ( ((n)>>(b)) | ((n)<<(32 - (b))) )
#define RRX(c,m) ( ((c)<<32)|((m)>>1) )
...
/* MISC INSTRUCTIONS TODO */
struct op_datap_rshift_t{
    unsigned Rm : 4;
    unsigned one : 1;
    unsigned shift : 2;
    unsigned zero : 1;
...
/* MISC INSTRUCTIONS TODO */
/* MULTIPLIES EXTRA LOAD/STORES TODO */
struct op_datap_immediate_t{
    unsigned immed_8 : 8;
...
struct op_MSR_t{
    unsigned immediate_8 : 8;
    unsigned rotate_imm : 4;
    unsigned SBO : 4;
    unsigned field_mask : 4;
    unsigned z0 : 1;
    unsigned n0 : 1;
    unsigned R : 1;
    unsigned mask : 5;
...
/* total space used? 32 bits or 32 bits * x ? */
union opcode_u{
    u32 value;
    ...
    struct op_datap_ishift_t DATAIS;
    struct op_datap_rshift_t DATARS;
    struct op_datap_immediate_t DATAI;
    struct op_load_store_ioff_t LSTRI;
    struct op_MSR_t     OP_MSR;
    struct {
        signed jump : 24;
        unsigned : 8;
    };
